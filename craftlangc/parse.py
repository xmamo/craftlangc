...
* Each function parses a single feature of the Craftlang language.  Some features (like functions and expressions) are
  inherently more complex and need bigger methods and/or the use of helper functions.  These conventions apply to
  helper methods as well;
...
* If the cursor is advanced beyond the parsed feature in order to look ahead, the cursor has to be restored at the end
  of the feature before the method returns.
'''
...
def parse_statement(walker: Walker, current_indent: int) -> Statement:
	...
	# If we got ``*=``, ``/=``, ``%=``, ``+=``, ``-=``, ``&=``, ``^=`` or ``|=`` after the identifier, we have an
	# assignment
	if ahead2 in {'*=', '/=', '%=', '+=', '-=', '&=', '^=', '|='}:
		pos = walker.pos
		walker.advance(2)
	...
	# If the identifier is ``nop`` and we have no other characters on the same line, we deduce that the identifier is
	# in fact a nop keyword, thus giving us a nop statement
	if identifier_lexeme == 'nop':
		ahead2 = walker.ahead()
		if is_newline(ahead2) or ahead2 == '':
			return NopStatement()

	# If we got a ``(`` after the identifier, we probably have a function call.  There's an ambiguity if identifier is
	# ``if`` or ``while``: do we have a function call or an if/while statement?
	if ahead == '(':
	...
	if identifier_lexeme == 'if':
		...
		# We advance the position of our walker, looking for a lonely ``else`` on the next line.  If there is this
		# ``else``, we also parse the else part of the if statement; otherwise, we reset the position of our walker to
		# where it was before.
		if_false = None
		pos = walker.pos
		walker.match(is_whitespace)
	...
	# We give up: none of the conditions above have been fulfilled.  We reset the walker position to where it was
	# before this method got called and we raise an error.
